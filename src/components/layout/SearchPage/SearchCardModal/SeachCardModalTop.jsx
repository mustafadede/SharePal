...
function SeachCardModalTop({ poster, backdrop, title, releaseDate, genres, mediaType, upcoming }) {
  return (
    <div className="relative flex flex-row-reverse w-full h-full md:h-[21rem]">
      {/* Image */}
      <motion.div
        className="absolute z-10 h-full w-48 left-1/4 md:left-auto top-20 md:top-12 md:w-52 md:right-20 drop-shadow-2xl"
      ...
      <div className="absolute top-0 right-0 w-full h-full overflow-hidden brightness-75 blur-md rounded-tr-2xl">
        <img src={`https://image.tmdb.org/t/p/w500/${backdrop}`} className="w-full object-cover h-full scale-y-150" alt={poster} />
      </div>
      {/* Title & Release Date */}
      <div className="absolute top-40 text-center left-16 md:text-left md:top-0 md:left-0 flex flex-col justify-center items-center md:items-start w-2/3 h-full p-6">
        ...
        <div className="relative flex flex-nowrap items-center md:flex-wrap gap-2 top-4">
          {mediaType === "movie" &&
            genres
